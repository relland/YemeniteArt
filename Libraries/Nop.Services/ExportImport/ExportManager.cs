...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        protected virtual void WriteCategories(XmlWriter xmlWriter, int parentCategoryId)
        {
            ...
            if (categories != null && categories.Any())
            {
                foreach (var category in categories)
                {
                    ...
                    //rel
                    xmlWriter.WriteElementString("PictureBId", null, category.PictureBId.ToString());
        ...
        //rel
        private byte[] REL_ExportProductsToXlsxWithAttributes(PropertyByName<Product>[] properties, IEnumerable<Product> itemsToExport)
        {
            var attributeProperties = new[]
            {
                new PropertyByName<ExportProductAttribute>("AttributeId", p => p.AttributeId),
                new PropertyByName<ExportProductAttribute>("AttributeName", p => p.AttributeName),
                //rel                
                new PropertyByName<ExportProductAttribute>("HasCondition", p => p.HasCondition),
                new PropertyByName<ExportProductAttribute>("RowValueIdForCondition", p => p.RowValueIdForCondition),
                new PropertyByName<ExportProductAttribute>("RowValueIsSelected", p => p.RowValueIsSelected),
                new PropertyByName<ExportProductAttribute>("RowValueId", p => p.RowValueId),
                //rel
                new PropertyByName<ExportProductAttribute>("AttributeTextPrompt", p => p.AttributeTextPrompt),
                //rel                
                new PropertyByName<ExportProductAttribute>("IsStoneAttribute", p => p.IsStoneAttribute),
                //rel
                new PropertyByName<ExportProductAttribute>("AttributeIsRequired", p => p.AttributeIsRequired),
                new PropertyByName<ExportProductAttribute>("AttributeControlType", p => p.AttributeControlTypeId)
                {
                ...
                new PropertyByName<ExportProductAttribute>("StoneNameId", p => p.StoneNameId),
                new PropertyByName<ExportProductAttribute>("BigPicture", p => p.BigPicture),
                new PropertyByName<ExportProductAttribute>("SmallPicture", p => p.SmallPicture)
                ...
                using (var xlPackage = new ExcelPackage(stream))
                {
                    ...
                    foreach (var item in itemsToExport)
                    {
                        ...
                        var attributes = item.ProductAttributeMappings.SelectMany(pam => pam.ProductAttributeValues.Select(pav => new ExportProductAttribute
                        {
                            ...
                            AttributeIsRequired = pam.IsRequired,
                            AttributeControlTypeId = pam.AttributeControlTypeId,
                            IsStoneAttribute = pam.IsStoneAttribute,
                            ...
                            BigPicture = GetLoacalImagePath(pav.PictureId),
                            SmallPicture = GetLoacalImagePath(pav.ImageSquaresPictureId),
                            
                            RowValueId = pav.Id,
                            HasCondition = !string.IsNullOrWhiteSpace(pam.ConditionAttributeXml),
                            RowValueIdForCondition = SelectedAttributeValueForCondition(pam.ConditionAttributeXml) > 0 ? SelectedAttributeValueForCondition(pam.ConditionAttributeXml) : !string.IsNullOrWhiteSpace(pam.ConditionAttributeXml) ? GetDefaultAttributeValue(pam.ConditionAttributeXml) : 0,
                            RowValueIsSelected = !string.IsNullOrWhiteSpace(pam.ConditionAttributeXml) && SelectedAttributeValueForCondition(pam.ConditionAttributeXml) == 0
                        })).ToList();
        ...
        //rel
        private string GetLoacalImagePath(int pictureId)
        {
            if (pictureId > 0)
            {
                var picture = _pictureService.GetPictureById(pictureId);
        ...
        //rel
        private int SelectedAttributeValueForCondition(string conditionAttributeXml)
        {
            if (!String.IsNullOrEmpty(conditionAttributeXml))
            {
                //select new values
                var selectedValues = _productAttributeParser.ParseProductAttributeValues(conditionAttributeXml);
                if (selectedValues.Count() > 0)
                {
                    var val = selectedValues.FirstOrDefault().Id;
                    return val;
                    //if (val == 0)
                }
        ...
        private int GetDefaultAttributeValue(string conditionAttributeXml)
        {
            if (!String.IsNullOrEmpty(conditionAttributeXml))
            {
                //select new values
                var selectedProductAttributeMappings = _productAttributeParser.ParseProductAttributeMappings(conditionAttributeXml);
                if (selectedProductAttributeMappings.Count() > 0)
                {
                    if (selectedProductAttributeMappings.FirstOrDefault().ProductAttributeValues.Count > 0)
                    return selectedProductAttributeMappings.FirstOrDefault().ProductAttributeValues.FirstOrDefault().Id;
                    //if (val == 0)
                }
                //foreach (var attributeValue in selectedValues)
                //    foreach (var item in attributeModel.Values)
        ...
        public virtual byte[] ExportCategoriesToXlsx(IEnumerable<Category> categories)
        {
            ...
            var properties = new[]
            {
                ...
                //rel
                new PropertyByName<Category>("PictureB", p => GetPictures(p.PictureBId)),
        ...
        /// </summary>
        /// <param name="products">Products</param>
        public virtual byte[] REL_ExportProductsToXlsx(IEnumerable<Product> products)
        {
            var properties = new[]
            {
                new PropertyByName<Product>("ProductType", p => p.ProductTypeId, IgnoreExportPoductProperty(p => p.ProductType))
            ...
            {
                if (productAdvancedMode || _productEditorSettings.ProductAttributes)
                    return REL_ExportProductsToXlsxWithAttributes(properties, productList);
            }
