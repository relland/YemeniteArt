...
namespace Nop.Admin.Controllers
{
    public partial class CategoryController : BaseAdminController
    {
        ...
        protected virtual void UpdatePictureSeoNames(Category category)
        {
            ...
            if (pictureB != null)
                _pictureService.SetSeoFilename(pictureB.Id, _pictureService.GetPictureSeName(category.Name));
        }
        ...
        public ActionResult Edit(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //rel
                int prevPictureBId = category.PictureBId;
                ...
                //rel
                if (prevPictureBId > 0 && prevPictureBId != category.PictureBId)
                {
                    var prevPictureB = _pictureService.GetPictureById(prevPictureBId);
                    if (prevPictureB != null)
                        _pictureService.DeletePicture(prevPictureB);
                }
                //update picture seo file name
                UpdatePictureSeoNames(category);
                //ACL
        ...
        [HttpPost]
        public ActionResult RELImportFromXlsx()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
