...
namespace Nop.Plugin.BrainStation.QuickView.Controllers
{
    public class BsQuickViewController : BasePublicController
    {
        ...
        private readonly int _comparisonImageSize;
            ...
            )
        {
            ...
            this._comparisonImageSize = Int32.Parse(ConfigurationManager.AppSettings["ComparisonImageSize"]);
        }
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                var defaultPicture = pictures.Skip(1).FirstOrDefault();
                var comparisonPicture = pictures.Skip(2).FirstOrDefault();
                //end rel
                ...
                //rel
                var comparisonPictureModel = new PictureModel
                {
                    ImageUrl = _pictureService.GetPictureUrl(comparisonPicture, _comparisonImageSize, !isAssociatedProduct),
                    FullSizeImageUrl = _pictureService.GetPictureUrl(comparisonPicture, 0, !isAssociatedProduct),
                    Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), model.Name),
                    AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name),
                };
                //"title" attribute
                comparisonPictureModel.Title = (comparisonPicture != null && !string.IsNullOrEmpty(comparisonPicture.TitleAttribute)) ?
                    comparisonPicture.TitleAttribute :
                    string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), model.Name);
                //"alt" attribute
                comparisonPictureModel.AlternateText = (comparisonPicture != null && !string.IsNullOrEmpty(comparisonPicture.AltAttribute)) ?
                    comparisonPicture.AltAttribute :
                    string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name);
                //end rel
                ...
                return new { DefaultPictureModel = defaultPictureModel, PictureModels = pictureModels, ComparisonPictureModel = comparisonPictureModel };
            });
            model.DefaultPictureModel = cachedPictures.DefaultPictureModel;
            model.PictureModels = cachedPictures.PictureModels;
            //rel
            model.ComparisonPictureModel = cachedPictures.ComparisonPictureModel;
            //end rel
            ...
            foreach (var attribute in productAttributeMapping)
            {
                var attributeModel = new ProductDetailsModel.ProductAttributeModel
                {
                    ...
                    IsStoneAttribute = attribute.IsStoneAttribute
                };
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        if (attributeValue.PictureId > 0)
                            valueModel.PictureUrl = _pictureService.GetPictureUrl(attributeValue.PictureId, _mediaSettings.ProductDetailsPictureSize);
                        //end rel
                    }
            ...
            #region Silver GF 14K 18K SeNames - links
            //rel
            model.DisplaySilverGF14K18K = product.DisplaySilverGF14K18K;
            model.SilverGF14K18KSeNames = new List<string>();
            if (model.DisplaySilverGF14K18K)
            {
                var seNameGF = "";
                var seName14K = "";
                var seName18K = "";
                var silverSeName = "";
                var baseSilverSeName = "";
                if (model.SeName.Contains("GF") || model.SeName.Contains("gf"))
                {
                    baseSilverSeName = model.SeName.Contains("GF") ? model.SeName.Replace("GF", "") : model.SeName.Replace("gf", "");
                    silverSeName = _productService.GetProductBySku(baseSilverSeName)?.Sku;
                    seName14K = _productService.GetProductBySku(baseSilverSeName + "14K")?.Sku;
                    seName18K = _productService.GetProductBySku(baseSilverSeName + "18K")?.Sku;
                    seNameGF = model.SeName;
                }
                else if (model.SeName.Contains("14K") || model.SeName.Contains("14k"))
                {
                    baseSilverSeName = model.SeName.Contains("14K") ? model.SeName.Replace("14K", "") : model.SeName.Replace("14k", "");
                    silverSeName = _productService.GetProductBySku(baseSilverSeName)?.Sku;
                    seNameGF = _productService.GetProductBySku(baseSilverSeName + "GF")?.Sku;
                    seName18K = _productService.GetProductBySku(baseSilverSeName + "18K")?.Sku;
                    seName14K = model.SeName;
                }
                else if (model.SeName.Contains("18K") || model.SeName.Contains("18k"))
                {
                    baseSilverSeName = model.SeName.Contains("18K") ? model.SeName.Replace("18K", "") : model.SeName.Replace("18k", "");
                    silverSeName = _productService.GetProductBySku(baseSilverSeName)?.Sku;
                    seNameGF = _productService.GetProductBySku(baseSilverSeName + "GF")?.Sku;
                    seName14K = _productService.GetProductBySku(baseSilverSeName + "14K")?.Sku;
                    seName18K = model.SeName;
                }
                else
                {
                    silverSeName = model.SeName;
                    seNameGF = _productService.GetProductBySku(silverSeName + "GF")?.Sku;
                    seName14K = _productService.GetProductBySku(silverSeName + "14K")?.Sku;
                    seName18K = _productService.GetProductBySku(silverSeName + "18K")?.Sku;
                }
                model.SilverGF14K18KSeNames.AddRange(new List<string> { silverSeName, seNameGF, seName14K, seName18K });
            }
            model.CssStoneClass = product.CssStoneClass;
            //end rel
            #endregion
        ...
        //rel
        [NonAction]
        protected virtual ProductDetailsModel PrepareProductDetailsPageModelOriginal(Product product,
                   ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            if (product == null)
                throw new ArgumentNullException("product");
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        if (attributeValue.PictureId > 0)
                        {
                            var productAttributePictureCacheKey = string.Format(ModelCacheEventConsumer.PRODUCTATTRIBUTE_PICTURE_MODEL_KEY,
                                ...
                                _webHelper.IsCurrentConnectionSecured(),
                                _storeContext.CurrentStore.Id);
                            valueModel.ImageSquaresPictureModel = _cacheManager.Get(productAttributePictureCacheKey, () =>
                            {
        ...
        public ActionResult AddButton()
        {
            ...
            return View("~/Plugins/BrainStation.QuickView/Views/QuickViewButton.cshtml", settingsModel);
        ...
        public ActionResult ProductDetails(int productId, int updatecartitemid = 0)
        {
            ...
            if (model.ProductTemplateViewPath.Equals("ProductTemplate.Simple"))
            {
                return Json(new
                {
                    html = this.RenderPartialViewToString("~/Plugins/BrainStation.QuickView/Views/QuickViewProductTemplate.Simple.cshtml", model),
                });
            ...
            else if (model.ProductTemplateViewPath.Equals("ProductTemplate.Grouped"))
            {
                return Json(new
                {
                    html = this.RenderPartialViewToString("~/Plugins/BrainStation.QuickView/Views/QuickViewProductTemplate.Grouped.cshtml", model),
                });
        ...
        //rel
        [NopHttpsRequirement(SslRequirement.No)]
        public ActionResult ProductDetailsOriginal(int productId, int updatecartitemid = 0)
        {
            var product = _productService.GetProductById(productId);
            if (product == null || product.Deleted)
                return InvokeHttp404();
            ...
            if (model.ProductTemplateViewPath.Equals("ProductTemplate.Simple"))
            {
               return Json(new
                {
                    html= this.RenderPartialViewToString("~/Plugins/BrainStation.QuickView/Views/QuickViewProductTemplate.Simple.cshtml", bsQuickViewModel),
                });
            ...
            else if (model.ProductTemplateViewPath.Equals("ProductTemplate.Grouped"))
            {
                return Json(new
                {
                    html = this.RenderPartialViewToString("~/Plugins/BrainStation.QuickView/Views/QuickViewProductTemplate.Grouped.cshtml", bsQuickViewModel),
                });
        ...
        public ActionResult RelatedProducts(int productId, int? productThumbPictureSize)
        {
            ...
            return PartialView("~/Plugins/BrainStation.QuickView/Views/QuickViewRelatedProducts.cshtml", model);
        }
        ...
        public ActionResult ProductsAlsoPurchased(int productId, int? productThumbPictureSize)
        {
            ...
            return PartialView("~/Plugins/BrainStation.QuickView/Views/QuickViewProductsAlsoPurchased.cshtml", model);
        }
        ...
        {
            return View("~/Plugins/BrainStation.QuickView/Views/BSQuickViewConfigure.cshtml");
        }
